...
import { listKikoBooksJournalEntries } from "../handlers/list-kikobooks-journal-entries.handler.js";
...
const toolHandler = async (args: any) => {
    const response = await listKikoBooksJournalEntries(args);
    if (response.isError) {
        return { content: [{ type: "text" as const, text: `Error listing journal entries: ${response.error}` }] };
    }
    return {
        content: [
...
export const ListJournalEntriesTool: ToolDefinition<typeof toolSchema> = {
