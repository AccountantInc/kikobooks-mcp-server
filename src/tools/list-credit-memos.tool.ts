...
import { listKikoBooksCreditMemos } from "../handlers/list-kikobooks-credit-memos.handler.js";
...
const toolHandler = async (args: any) => {
    const response = await listKikoBooksCreditMemos(args);

    if (response.isError) {
        return {
            content: [{ type: "text" as const, text: `Error listing credit memos: ${response.error}` }],
        };
    }
...
export const ListCreditMemosTool: ToolDefinition<typeof toolSchema> = {
