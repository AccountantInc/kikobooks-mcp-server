...
import { createKikoBooksVendorPayment } from "../handlers/create-kikobooks-vendor-payment.handler.js";
...
const toolHandler = async (args: any) => {
    const response = await createKikoBooksVendorPayment(args.payment);

    if (response.isError) {
        return {
            content: [{ type: "text" as const, text: `Error creating vendor payment: ${response.error}` }],
        };
    }

    return {
        content: [
            { type: "text" as const, text: "Vendor Payment created:" },
            { type: "text" as const, text: JSON.stringify(response.result, null, 2) },
        ],
    };
};

export const CreateVendorPaymentTool: ToolDefinition<typeof toolSchema> = {
    name: "create_vendor_payment",
    description:
        "Create a vendor payment (AP) in KikoBooks. Requires vendor ID, payment date, source account, and amount. Optionally apply to specific bills.",
    schema: toolSchema,
    handler: toolHandler,
};
